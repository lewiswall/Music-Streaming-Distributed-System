        ...
        while received_data:
            ...
            if not tokenAccepted:
                if received_data == 'notLoggedIn':
                    ...
                    connections -= 1
                    signup = ServiceFuncs.AskForAddr(primeNode, 'SignUp').split(':')
                    message = 'connect,' + signup[0] + ',' + signup[1] + ',' + signup[2]
                    peer_socket.send(message.encode("utf-8"))
                elif received_data == 'l' or received_data == 'L':
                    connections -= 1
                    address = ServiceFuncs.AskForAddr(primeNode, 'Login').split(':')
                ...
                if received_data == correctUser['user']:
                    ...
                    serviceNode = ServiceFuncs.AskForAddr(primeNode, 'ServiceNode').split(':')
...
def listen() -> None:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        ServiceFuncs.RegisterWithPrime(primeNode, 'ControlNode', server_host, server_port)
        ...
        while True:
            ...
            if connections == 3:
                if not loadBalancer:
                    ServiceFuncs.RegisterAsFull(primeNode, 'ControlNode', server_host, server_port)
                    loadBalancer = True
