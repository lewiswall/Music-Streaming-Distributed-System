...
if __name__ == "__main__":
    ...
    def listen() -> None:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
            ...
            registerwithprime(primeNode, loadBalancerType, server_host, server_port)
    ...
    def handleprime():
        ...
        parentNode = {'type': prime[0],
                      'host': prime[1],
                      'port': int(prime[2])}
        return parentNode

    def registerwithprime(prime, selftype, host, port):
        global addrs
        message = 'LoadBalancer:' + selftype + ':' + host + ':' + str(port)
        # return message will be the addresses of the service for the load balancer
        returnMessage = sendmessage(prime, message)
        addrs = eval(returnMessage)
